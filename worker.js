...
class EnvironmentConfig {
  ...
  DEBUG_MODE = false;
  DEV_MODE = false;
  PROMPT = "你是一个得力的助手"; // 新增全局系统prompt
}
const zhHans = { // 只保留简体中文
  ...
  "command": {
    "help": {
      ...
      "echo": "回显消息",
      "models": "切换对话模型",
      "prompt": "自定义系统Prompt，完整命令格式为 `/prompt {内容}` 或 `/prompt reset` 重置"
    },
...
async function requestCompletionsFromLLM(params, context, agent, modifier, onStream) {
  ...
  const sysPrompt = context.USER_CONFIG.SYSTEM_INIT_MESSAGE || ENV.PROMPT;
  const llmParams = {
    prompt: sysPrompt,
...
class PromptCommandHandler {
  ...
  handle = async (message, subcommand, context) => {
    const sender = MessageSender.fromMessage(context.SHARE_CONTEXT.botToken, message);
    if (!subcommand) {
      let currentPrompt = context.USER_CONFIG.SYSTEM_INIT_MESSAGE || ENV.PROMPT;
      return sender.sendPlainText(`当前系统Prompt：\n${currentPrompt}\n\n使用 /prompt {内容} 设置新Prompt，或 /prompt reset 清空`);
    }
    if (subcommand.trim().toLowerCase() === "reset") {
      context.USER_CONFIG.SYSTEM_INIT_MESSAGE = null;
      await ENV.DATABASE.put(
        context.SHARE_CONTEXT.configStoreKey,
        JSON.stringify(ConfigMerger.trim(context.USER_CONFIG, ENV.LOCK_USER_CONFIG_KEYS))
      );
      return sender.sendPlainText(`已重置为全局系统Prompt：\n${ENV.PROMPT}`);
    }
    context.USER_CONFIG.SYSTEM_INIT_MESSAGE = subcommand;
    await ENV.DATABASE.put(
      context.SHARE_CONTEXT.configStoreKey,
      JSON.stringify(ConfigMerger.trim(context.USER_CONFIG, ENV.LOCK_USER_CONFIG_KEYS))
    );
    return sender.sendPlainText(`已设置新的系统Prompt：\n${subcommand}`);
  }
}
